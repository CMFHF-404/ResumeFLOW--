...
root=Path('.artifacts/score-unlimited-20260914')
...
manifest=dict(startedAt=datetime.now(timezone.utc).isoformat(),model=settings.resume_score_model,thinking=settings.resume_score_thinking_level,promptVersion=object_review.PROMPT_VERSION,evidenceOutputVersion=object_review.EVIDENCE_OUTPUT_VERSION,timeoutSeconds=None,inputSha256=hashlib.sha256((root/'input.json').read_bytes()).hexdigest(),codeHashes=hashes)
