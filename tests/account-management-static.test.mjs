...
test('account management feature is wired through sidebar, app shell, service, and env', () => {
  ...
  assert.match(modal, /touch-pan-y/);
  assert.match(modal, /\[-webkit-overflow-scrolling:touch\]/);
  assert.match(modal, /px-3 py-3 sm:px-5 sm:py-4/);
  assert.match(modal, /<h2[\s\S]*账号管理[\s\S]*<\/h2>[\s\S]*<VerificationBadge record=\{identityVerification\} \/>/);
  assert.match(modal, /grid-cols-2[\s\S]*<AccountSummaryItem label="邮箱"[\s\S]*<AccountSummaryItem label="手机号"/);
  assert.doesNotMatch(modal, /<div className="text-xs text-slate-500">二次验证<\/div>[\s\S]*<VerificationBadge record=\{identityVerification\} \/>/);
  assert.match(modal, /aria-label="账号更新步骤"/);
  assert.match(modal, /absolute left-0 right-0 top-\[14px\]/);
  assert.doesNotMatch(modal, /rounded-lg border px-2\.5 py-1\.5 text-xs font-medium transition/);
  assert.match(modal, /<div className=\{`\$\{PANEL_CLASS\} relative`\}>[\s\S]*absolute right-3 top-3/);
  assert.doesNotMatch(modal, /<button className=\{SECONDARY_BUTTON_CLASS\} onClick=\{returnToSelection\}/);
  assert.match(modal, /p-3[\s\S]*sm:p-4/);
