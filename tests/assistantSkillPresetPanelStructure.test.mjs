...
test('AIAssistant binds composer quick buttons to assistant skill presets', () => {
  ...
  const selectedResumeContextHook = read('views/AIAssistant/useAssistantSelectedResumeContext.ts');
  const sessionLoadingHook = read('views/AIAssistant/useAssistantSessionLoading.ts');
  ...
  assert.doesNotMatch(assistant, /继续描述细节或调整内容/);
  assert.doesNotMatch(assistant, /例如：我想整理一段校园运营经历/);
  assert.match(assistant, /if \(activeComposerSkillId === skillId\) \{/);
  assert.match(assistant, /setActiveComposerSkillId\(null\)/);
  assert.match(assistant, /current === prompt \? '' : current/);
  assert.match(assistant, /from '\.\/AIAssistant\/AssistantContextRail'/);
  assert.match(assistant, /from '\.\/AIAssistant\/useAssistantSelectedResumeContext'/);
  assert.match(assistant, /useAssistantSelectedResumeContext\(\)/);
  assert.doesNotMatch(assistant, /const selectedResumeIdRef = useRef<string \| null>\(null\)/);
  assert.doesNotMatch(assistant, /buildSelectedResumeForTurn\(/);
  assert.match(selectedResumeContextHook, /from '\.\/resumeSelectionUtils'/);
  assert.match(selectedResumeContextHook, /const \[selectedResumeModuleIds, setSelectedResumeModuleIds\] = useState<string\[\]>\(\[\]\)/);
  assert.match(selectedResumeContextHook, /const selectedResumeIdRef = useRef<string \| null>\(null\)/);
  assert.match(selectedResumeContextHook, /const currentSelectedResumeId = selectedResume\?\.resumeId \?\? null/);
  assert.match(selectedResumeContextHook, /selectedResumeIdRef\.current = currentSelectedResumeId/);
  assert.match(selectedResumeContextHook, /setSelectedResumeModuleIds\(\[\]\)/);
  assert.match(selectedResumeContextHook, /displayLabel: exp\.org \|\| exp\.title \|\| '经历'/);
  assert.match(selectedResumeContextHook, /displayLabel: edu\.school \|\| '教育经历'/);
  assert.match(selectedResumeContextHook, /displayLabel: cert\.name \|\| '证书资质'/);
  assert.match(selectedResumeContextHook, /kind: 'experience'/);
  assert.match(selectedResumeContextHook, /contextId: exp\.id/);
  assert.match(selectedResumeContextHook, /const selectedResumeModulesForTurn = useMemo/);
  assert.match(selectedResumeContextHook, /const selectedIdSet = new Set\(selectedResumeModuleIds\)/);
  assert.match(selectedResumeContextHook, /\.filter\(\(item\) => selectedIdSet\.has\(item\.id\)\)/);
  assert.match(selectedResumeContextHook, /const selectedResumeForTurn = useMemo/);
  assert.match(selectedResumeContextHook, /buildSelectedResumeForTurn\(selectedResume, selectedResumeModulesForTurn\)/);
  assert.match(selectedResumeContextHook, /restoreSelectedResumeContext/);
  assert.match(selectedResumeContextHook, /selectedResumeIdRef\.current = nextSelectedResume\?\.resumeId \?\? null/);
  assert.match(sessionLoadingHook, /restoreSelectedResumeContext\(restoredContext\)/);
  assert.doesNotMatch(sessionLoadingHook, /setSelectedResumeModuleIds\(restoredContext\.selectedResumeModuleIds\)/);
  ...
  assert.match(conversationViewport, /isSending/);
  ...
  assert.match(chatInput, /suggestedFollowups\?: AssistantSuggestedFollowup\[\]/);
  assert.match(chatInput, /onSelectSuggestedFollowup\?: \(skillId: AssistantSkillId, prompt: string\) => void/);
  assert.match(chatInput, /const hasSuggestedFollowups = suggestedFollowups\.length > 0;/);
  assert.match(chatInput, /flex-nowrap overflow-x-auto/);
  assert.match(chatInput, /suggestedFollowups\.map\(\(item\) =>/);
  assert.match(chatInput, /onSelectSuggestedFollowup\?\.\(item\.skillId, item\.prompt\)/);
  assert.match(chatInput, /className="inline-flex shrink-0 items-center rounded-full/);
  assert.match(chatInput, /className="whitespace-nowrap"/);
  assert.doesNotMatch(chatInput, /followupScrollRef/);
  assert.doesNotMatch(chatInput, /followupScrollMetrics/);
  assert.doesNotMatch(chatInput, /shouldShowFollowupScrollbar/);
  assert.doesNotMatch(chatInput, /updateFollowupScrollMetrics/);
  assert.doesNotMatch(chatInput, /role="scrollbar"/);
  assert.doesNotMatch(chatInput, /aria-label="横向滚动建议选项"/);
  assert.doesNotMatch(chatInput, /handleFollowupScrollbarPointerDown/);
  assert.doesNotMatch(chatInput, /handleFollowupScrollbarKeyDown/);
  assert.doesNotMatch(chatInput, /\[scrollbar-width:none\]/);
  assert.doesNotMatch(chatInput, /\[-ms-overflow-style:none\]/);
  assert.doesNotMatch(chatInput, /\[&::-webkit-scrollbar\]:hidden/);
  assert.doesNotMatch(chatInput, /whitespace-normal break-words/);
  ...
  assert.doesNotMatch(chatInput, /深度思考|BrainCircuit|onDeepThinkingChange/);
