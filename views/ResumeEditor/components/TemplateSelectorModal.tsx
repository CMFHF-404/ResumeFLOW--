...
export const TemplateThumbnail: React.FC<{
  ...
  if (template.layoutKind === 'classic') {
    ...
    const isOpenSourceClassic = templateId === 'open-source-classic';
    if (isOpenSourceClassic) {
      return (
        <div className="h-full rounded-lg border border-gray-200 bg-white px-3 py-3 dark:border-gray-700 dark:bg-gray-950">
          <div className="mb-3 text-center">
            <div className="mx-auto mb-1 h-2.5 w-20 rounded bg-gray-900/85" />
            <div className="mx-auto h-1 w-24 rounded bg-gray-200" />
          </div>
          <div className="space-y-2.5">
            {[0, 1, 2].map((item) => (
              <div key={item} className="grid grid-cols-[2.25rem_1fr] gap-2">
                <div className="h-1.5 w-8 rounded-full" style={{ backgroundColor: theme.accentColor }} />
                <div>
  ...
  if (template.layoutKind === 'accent') {
    const isTimelineBlue = templateId === 'timeline-blue';
    if (isTimelineBlue) {
      return (
        <div className="relative h-full overflow-hidden rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-950">
          <div className="mb-3 flex items-start justify-between gap-2">
            <div>
              <div className="mb-1.5 h-2.5 w-16 rounded bg-gray-900/80" />
              <div className="h-1 w-24 rounded bg-gray-200" />
            </div>
            <div className="h-8 w-8 rounded-full" style={{ backgroundColor: theme.accentSoftBg, border: `1px solid ${theme.accentBorder}` }} />
          </div>
          <div className="space-y-2.5">
            {[0, 1, 2].map((item) => (
              <div key={item} className="relative pl-4">
                <div className="absolute bottom-[-10px] left-[5px] top-2 w-[1px]" style={{ backgroundColor: theme.accentBorder }} />
                <div className="absolute left-0 top-1.5 h-2.5 w-2.5 rounded-full" style={{ backgroundColor: theme.accentColor }} />
  ...
  if (template.layoutKind === 'avatar') {
    const isPhotoCard = templateId === 'photo-card';
    if (isPhotoCard) {
      return (
        <div className="h-full rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-950">
          <div className="mb-3 rounded-lg p-2" style={{ backgroundColor: theme.accentSoftBg }}>
            <div className="flex items-center gap-2">
              <div className="h-12 w-9 rounded-md border border-white bg-white/80" />
  ...
  const isPhotoSidebar = templateId === 'photo-sidebar';
  return (
    <div className="h-full overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-950">
      <div className="grid h-full grid-cols-[0.7fr_1.3fr]">
        <div className="p-3" style={{ backgroundColor: isPhotoSidebar ? '#111827' : theme.accentSoftBg }}>
          <div className={`mb-2 h-10 w-7 ${isPhotoSidebar ? 'rounded-full border border-white/35 bg-white/20' : 'rounded-md border border-white/70 bg-white/80'}`} />
          <div className="mb-1 h-1.5 w-10 rounded-full" style={{ backgroundColor: isPhotoSidebar ? '#ffffff' : theme.accentColor }} />
          <div className="space-y-1">
            <div className={`h-1.5 w-full rounded ${isPhotoSidebar ? 'bg-white/45' : 'bg-white/80'}`} />
            <div className={`h-1.5 w-4/5 rounded ${isPhotoSidebar ? 'bg-white/35' : 'bg-white/80'}`} />
            <div className={`h-1.5 w-3/5 rounded ${isPhotoSidebar ? 'bg-white/30' : 'bg-white/80'}`} />
          </div>
        </div>
        <div className="p-3">
