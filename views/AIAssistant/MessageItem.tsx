  ...
  return (
    <div className="mb-3 flex gap-2 overflow-x-auto pb-1 no-scrollbar">
      {attachments.map((attachment) => (
        <div
          key={attachment.id}
          className="flex h-[80px] w-[204px] shrink-0 gap-3 rounded-2xl border border-slate-200 bg-white/90 px-3 py-3"
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 truncate text-xs font-semibold leading-5 text-slate-700">
              {attachment.name}
            </div>
      ...
      {selectedResume ? (
        <div className="flex h-[80px] w-[204px] shrink-0 gap-3 rounded-2xl border border-sky-200 bg-sky-50/75 px-3 py-3">
      ...
      {selectedExperiences.map((item) => {
        ...
        return (
          <div
            key={item.masterId}
            className="flex h-[80px] w-[204px] shrink-0 gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/75 px-3 py-3"
          >
          ...
          <div className="space-y-3 overflow-hidden break-words text-sm leading-7">
            <ReactMarkdown
              components={{
                ...
                ul: ({ node, ...props }) => <ul className="m-0 list-disc space-y-1.5 pl-5 marker:text-slate-400" {...props} />,
                ol: ({ node, ...props }) => <ol className="m-0 list-decimal space-y-1.5 pl-5 marker:font-medium marker:text-slate-400" {...props} />,
                ...
                h1: ({ node, ...props }) => <h1 className="mb-2 mt-4 text-lg font-bold text-slate-900" {...props} />,
                h2: ({ node, ...props }) => <h2 className="mb-2 mt-4 text-base font-bold text-slate-900" {...props} />,
                h3: ({ node, ...props }) => <h3 className="mb-1.5 mt-3 text-sm font-bold text-slate-900" {...props} />,
                blockquote: ({ node, ...props }) => <blockquote className="my-2 whitespace-pre-wrap border-l-4 border-slate-200 pl-4 italic text-slate-600" {...props} />,
                code: ({ node, inline, ...props }: any) => inline
                  ? <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[13px] text-slate-800" {...props} />
                  : <code className="my-2 block overflow-x-auto whitespace-pre rounded-lg border border-slate-100 bg-slate-50 p-3 font-mono text-[13px] text-slate-800 shadow-inner" {...props} />,
              }}
            >
              {content}
            </ReactMarkdown>
  ...
  return (
    <div className="mb-6 flex w-full min-w-0 justify-start gap-3 sm:gap-4">
      ...
      <div className="min-w-0 max-w-full sm:max-w-[85%]">
        ...
        {expanded ? (
          <div className="ml-1.5 mt-2 whitespace-pre-wrap border-l-2 border-slate-200 py-1 pl-4 text-sm leading-6 text-slate-500">
            {thought}
          </div>
        ) : null}
      </div>
