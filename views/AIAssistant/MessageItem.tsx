...
import { Bot, Loader2, ChevronDown, ChevronRight, Paperclip, Briefcase, FolderKanban, GraduationCap, FileText } from 'lucide-react';
...
export const MessageItem: React.FC<MessageItemProps> = ({ isUser, content, attachment, selectedExperiences = [], selectedResume = null }) => {
  if (isUser) {
    return (
      <div className="mb-4 flex w-full min-w-0 justify-end">
        <div className="w-fit max-w-[88%] min-w-0 rounded-2xl rounded-tr-sm bg-slate-100 px-4 py-3 text-slate-800 sm:max-w-[80%] sm:px-5">
          {selectedResume ? (
            <div className="mb-3 rounded-2xl border border-sky-200 bg-white/90 px-3 py-2">
              <div className="flex items-start gap-2">
                <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-sky-50 text-sky-600">
