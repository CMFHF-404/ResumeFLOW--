...
import type { ResumeEditorConfig, SmartPageLayout } from '../types/resume';
import {
...
import { normalizeSectionOrder } from '../utils/resumeSectionOrder';
...
const isAuthenticatedOwnerId = (
  ownerId: string | null | undefined
): ownerId is string => (
  typeof ownerId === 'string'
  && ownerId.trim().length > 0
  && ownerId.trim() !== 'anonymous'
);

export const buildPreferredResumeTemplateStorageKey = (ownerId: string) => (
  `${PREFERRED_RESUME_TEMPLATE_STORAGE_KEY}:${encodeURIComponent(ownerId)}`
);

export const buildResumeTemplatePresetStorageKey = (ownerId: string) => (
  `${RESUME_TEMPLATE_PRESETS_STORAGE_KEY}:${encodeURIComponent(ownerId)}`
);

const clearLegacyResumeTemplateStorage = () => {
  if (typeof window === 'undefined') {
    return;
  }
  window.localStorage.removeItem(PREFERRED_RESUME_TEMPLATE_STORAGE_KEY);
  window.localStorage.removeItem(RESUME_TEMPLATE_PRESETS_STORAGE_KEY);
};
...
const writeResumeTemplatePresetMapToStorage = (
  ...
  clearLegacyResumeTemplateStorage();
  if (!isAuthenticatedOwnerId(ownerId)) {
    return;
  }
  window.localStorage.setItem(
    buildResumeTemplatePresetStorageKey(ownerId),
    JSON.stringify(serializeResumeTemplatePresetMap(presetMap))
  );
};

export const loadPreferredResumeTemplateId = (
  ownerId: string | null | undefined
): ResumeTemplateId => {
  ...
  const storageKey = buildPreferredResumeTemplateStorageKey(ownerId);
...
export const savePreferredResumeTemplateId = (
  templateId: ResumeTemplateId,
  ownerId: string | null | undefined
) => {
  if (typeof window === 'undefined') {
  ...
  window.localStorage.setItem(
    buildPreferredResumeTemplateStorageKey(ownerId),
...
export const loadResumeTemplatePresetMap = (
  ...
  const storageKey = buildResumeTemplatePresetStorageKey(ownerId);
  const raw = window.localStorage.getItem(storageKey);
  if (!raw) {
    return {};
  }
...
export const saveResumeTemplatePreset = async (
  ...
  if (!isAuthenticatedOwnerId(ownerId)) {
    throw new Error('无法在未登录状态保存模板预设');
  }
  ...
  if (profile.user_id !== ownerId) {
    throw new Error('Authentication context changed while saving template preset');
  }
  ...
  if (updatedProfile.user_id !== ownerId) {
    throw new Error('Authentication context changed while saving template preset');
  }
  syncResumeTemplatePresetsFromProfile(updatedProfile.extra_json, ownerId);
...
export const buildPreferredResumeCreateConfig = (
  ...
  const templateId = loadPreferredResumeTemplateId(ownerId);
