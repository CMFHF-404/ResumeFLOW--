...
async def get_experience_detail(
    ...
    master = await get_master(session, user_id, master_id)
    result = await session.exec(
        select(ExperienceVersion)
...
async def create_version(
    session: AsyncSession, user_id: str, master_id: str, payload: ExperienceVersionPayload
) -> ExperienceVersion:
    master = await get_master(session, user_id, master_id)
    next_version = await _next_version_number(session, master.id)
    version = _build_version(master.id, next_version, payload)
    session.add(version)
    await session.flush()
