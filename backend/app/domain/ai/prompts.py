...
ASSISTANT_COMMON_RULES = (
    ...
    "If the input JSON includes 'bank_context', treat it as a bounded snapshot of the user's authoritative experience library and latest personal material. "
    "Always inspect bank_context._meta; when any list is marked truncated, do not infer that an omitted experience, certification, or skill does not exist. "
...
GENERAL_ASSISTANT_PROMPT = (
    ...
    "For education cards, map data.org to school, data.title to major, data.star.s to degree, data.star.t to GPA or grade point, and data.star.a to courses instead of action bullets. "
    "Keep course grades or scores on the same line as the course name, for example '测试课程（90）'; do not split, omit, or rewrite them as action bullets. "
    "Set data.star.r to an empty string for education cards. "
    ...
    "The data object must contain: category and skills. skills must be an array of objects with key name, and may include optional targetUserSkillId. "
    "When bank_context.skills or selected_resume.snapshot.skills clearly matches an existing skill, set targetUserSkillId to that existing user skill id. Never fabricate targetUserSkillId. "
    "Do not output numeric skill ratings or a proficiency field. If skill maturity matters, write it naturally in the skill name, e.g. '熟练掌握 Vibe Coding'. "
    "Use empty strings for unknown string fields and use false for unknown isCurrent. "
...
EXPERIENCE_ASSISTANT_PROMPT = (
    ...
    "When category is 'education', map org to school, title to major, star.s to degree, star.t to GPA or grade point, and star.a to courses instead of action bullets. "
    "Keep course grades or scores on the same line as the course name, for example '测试课程（90）'; do not split, omit, or rewrite them as action bullets. "
    "Set star.r to an empty string for education cards. "
    "If multiple courses are listed, separate them with '\\n' and do not add numbering or bullet prefixes. "
...
SKILL_ASSISTANT_PROMPT = (
    ...
    "skills must be an array of objects with key name, and may include optional targetUserSkillId. "
    "When bank_context.skills or selected_resume.snapshot.skills clearly matches an existing skill, set targetUserSkillId to that existing user skill id. Never fabricate targetUserSkillId. "
    "Do not output numeric skill ratings or a proficiency field. If skill maturity matters, write it naturally in the skill name, e.g. '熟练掌握 Vibe Coding'."
)
