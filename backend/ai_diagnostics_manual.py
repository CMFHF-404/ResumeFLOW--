...
async def check_gemini_thinking_stream(base_url, api_key, model):
    ...
    print(f"\n[2] Testing Gemini Thinking Stream: {url}")
    headers = {
...
async def main():
    ...
    gemini_key = os.getenv("GEMINI_API_KEY") or ai_key
    gemini_base = os.getenv("GEMINI_BASE_URL") or ai_base
    gemini_model = os.getenv("GEMINI_MODEL") or ai_model

    print("--- AI Service Diagnostics ---")
    print(f"Chat API: URL={ai_base}, Model={ai_model}, Key={mask_secret(ai_key)}")
    print(f"Gemini API: URL={gemini_base}, Model={gemini_model}, Key={mask_secret(gemini_key)}")
