...
- Single-page PDF changes use `node --test tests/resumePageGeometry.test.mjs tests/resumePaginationPolicy.test.mjs tests/resumeSmartPageLifecycle.test.mjs` and, from `backend/`, `python -B -m unittest test_resume_pdf_page_constraint test_resume_pdf_constraint_http`. Attach the one-page constraint only after the current layout is verified; the backend validates actual PDF pages. Local fixture checks do not establish authenticated export or deployed-browser acceptance.
- Expired resume-optimization planning reads must fail the locked run without changing its snapshot or billing and without starting AI work; use `python -B -m unittest test_resume_optimization_expired_planning` from `backend/`.

Default scoring is `resume_score_v2` / `single_pass_v1`; opt-in evidence scoring is `resume_score_v3` / `evidence_rubric_v1`, and human quality acceptance remains separate from feature activation. Preserve one scoring call, one selected-module planning call, at most one answered-module rewrite, no audit/repair calls or automatic post-apply scoring, and zero calls for empty selections or skipped answers. Use `node --test tests/resumeScore.test.mjs tests/evidenceResumeScore.test.mjs tests/evidenceResumeScore.browser.test.mjs` plus `python -B -m unittest test_resume_score test_evidence_resume_score test_evidence_score_evaluation` from `backend/` when these entrypoints change; historical compatibility tests must not restore model audits or content regex gates. Preserve auth, save/version, HTML sanitization and scoped writes.

For `review_json_schema_v4` / `json_structure_v3`, compatible skill creation and skill-order suggestions may batch, but each candidate still requires an explicit decision. Apply/revert must atomically preserve selection order and resume-local skills without model calls. Use `node --test tests/scoreSuggestionSelection.test.mjs tests/skillConfirmation.browser.test.mjs tests/resumeOptimizationNormalize.test.mjs` and `python -B -m unittest test_selection_actions_v4` from `backend/`.
